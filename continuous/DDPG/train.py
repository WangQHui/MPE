...
def train():
    ...
    # 初始化DDPGAgent
    ddpgagents = [DDPGAgent(name="agent_"+str(i),
    ...
    #print(critic.get_shape().as_list())
    ...
    while episode < train_parameters["num-episodes"]:
        ...
        for agent in ddpgagents:
            agent.action_noise.reset()

        while episode_steps < train_parameters["max-episode-len"]:
            T = time.time()
            # get action
            action_n = [agent.action(obs, evaluation=False) for agent, obs in zip(ddpgagents, obs_n)]
            # print(time.time()-T)
            # environment step
            # print(action_n)
            ...
            # collect experience
            for i, agent in enumerate(ddpgagents):
            ...
            if epoch % 50 == 0:
                ...
                train_able = False
                for i in range(env.n):
                    if len(ddpgagents[i].replay_buffer) > ddpgagents[i].max_replay_buffer_len:  # replay buffer is not large enough
                        train_able = True
                        ddpgagents[i].update(ddpgagents, train_step)
                if train_able:
                    train_step += 1
        ...
        for i, ddpgagent in enumerate(ddpgagents):
            with ddpgagent.summary_writer.as_default():
            ...
            ddpgagent.summary_writer.flush()

        if episode % 50 == 0:
            # 保存模型参数
            for agent in ddpgagents:
                agent.save_model("models/ddpg_actor_agent_", "models/ddpg_critic_agent_")

    # 关闭summary，回收资源
    for i, ddpgagent in enumerate(ddpgagents):
        ddpgagent.summary_writer.close()
    env.close()
    # 保存模型参数
    for agent in ddpgagents:
        agent.save_model("models/ddpg_actor_agent_", "models/ddpg_critic_agent_")
...
def inference(episode_num=100, max_episode_steps=100):
    ...
    # 初始化MADDPGAgent
    ddpgagents = [DDPGAgent(name="agent" + str(i),
                            obs_shape=env.observation_space,
                            act_space=env.action_space,
                            agent_num=env.n,
                            agent_index=i,
                  ...
                  for i in range(env.n)]
    for agent in ddpgagents:
        agent.load_actor(a_fn="models/ddpg_actor_agent_")
    ...
    while episode < episode_num:
        ...
        while step < max_episode_steps:
            # get action
            action_n = [agent.action(obs, evaluation=True) for agent, obs in zip(ddpgagents, cur_state)]
            # print(action_n)
            # environment step
