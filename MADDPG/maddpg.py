...
# Critic Model
def critic(state_shapes, action_dims, agent_num):
    inputs = []
    for a in range(agent_num):
        inputs.append(Input(shape=state_shapes[a]))
    for a in range(agent_num):
        inputs.append(Input(shape=action_dims[a]))
...
class MADDPGAgent():
    def __init__(self, name, obs_shape, act_space, agent_num, agent_index, parameters, create_summary_writer=False):
        ...
        self.critic = critic(state_shapes=[obs.shape[0]for obs in obs_shape], action_dims=[act.shape[0] for act in act_space], agent_num=agent_num)

        self.actor_target = actor(state_shape=obs_shape[self.agent_index].shape[0], action_dim=act_space[self.agent_index].shape[0])
        self.critic_target = critic(state_shapes=[obs.shape[0]for obs in obs_shape], action_dims = [act.shape[0] for act in act_space], agent_num = agent_num)
        ...
        if create_summary_writer:
            ...
            train_log_dir = 'logs/MADDPG_Summary_' + current_time + "agent" + str(self.agent_index)
    ...
    def update(self, agents, train_step):
        ...
        obs_next_n = tf.convert_to_tensor(obs_next_n, dtype=tf.float32)
    ...
    def train(self, memories):
        ...
        with tf.GradientTape() as tape:
            ...
            q_actor = self.critic(obs_n + act_n)
